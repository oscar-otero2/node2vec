...
PWNet RecvChunk(int *SelectedLen, int **SelectedBuff, double ParamP,
  ...
  *SelectedBuff = (int *)malloc(*SelectedLen * sizeof(int));
  ...
  int64 NCnt = 0; // This variable is not really used but needed
  // This is really the only needed change, the processing is different
  // But all of the sending and gathering is exactly the same
  TIntV NIds;
  ...
  // Init nodes because it was done in the parallel procedure
  for (TWNet::TNodeI NI = ProcNet->BegNI(); NI < ProcNet->EndNI(); NI++) {
  ...
  printf("Hey, starting processing\n\n");
  for (int64 i = 0; i < NIds.Len(); i++) {
    PreprocessNode(ProcNet, ParamP, ParamQ, ProcNet->GetNI(NIds[i]),
                   NCnt, false);
  }
  printf("Hey, ending processing\n\n");
  /*
...
void RecvResult(PWNet &InNet) {
  ...
  */
  ...
  for (int i = 0; i < SelectedLen; i++) {
    MPI_Recv(&Lens[i], 1, MPI_INT, Proc, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    // I need that i in order to alloc the buffer
    char *RecvBuff = (char *)malloc(Lens[i] * sizeof(char));
    MPI_Recv(RecvBuff, Lens[i], MPI_BYTE, Proc, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);

    TMIn inFor((const void *)RecvBuff, Lens[i], false); // stack TMIn
...
void PreprocessNode(PWNet &InNet, const double &ParamP, const double &ParamQ,
  ...
  for (int64 i = 0; i < NI.GetOutDeg(); i++) {
    ...
    printf("For neighbours' neighbour begins\n");
    for (int64 j = 0; j < CurrI.GetOutDeg(); j++) { // for each node x
      int64 FId = CurrI.GetNbrNId(j);
      TFlt Weight;
